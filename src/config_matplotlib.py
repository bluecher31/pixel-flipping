...
def line_with_shaded_errorband(ax: plt.Axes, x: NDArray, y: NDArray,
                               yerr: NDArray, label: str, **kwargs_plot) -> plt.matplotlib.lines.Line2D:
    """Plot error as a shaded region around the mean value."""
    [line_plot] = ax.plot(x, y, 'o-', label=label, **kwargs_plot)
    ax.fill_between(x, y - yerr, y + yerr, color=line_plot.get_color(), alpha=0.2)
    return line_plot


def clear_plot_figma():
    plt.xlabel('')
    plt.ylabel('')
    plt.legend([])

    # ax = plt.gca()
    # plt.tick_params(labelleft=False, labelbottom=False, bottom=True, top=False, right=False)
